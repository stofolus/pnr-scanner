class Pnr {
  static get regex() {
    return /\b(19|20)?[0-9]{2}[- ]?((0[0-9])|(10|11|12))[- ]?(([0-2][0-9])|(3[0-1])|(([7-8][0-9])|(6[1-9])|(9[0-1])))[- ]?[0-9]{4}\b/g;
  }
  ...
  static isValidPnr(pnr) {
    ...
    // 10 - sum % 10
    const controlNumber =
      10 - num.reduce((acc, curr) => acc + parseInt(curr, 10), 0) % 10;
    return parseInt(pnr[pnr.length - 1], 10) === controlNumber;
  }

  static normalizePnr(pnr) {
    let normalizedPnr = pnr.replace("-", "");
