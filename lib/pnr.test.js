...
describe(`regex`, () => {
  ...
  test(`Should not match if pnr is inside numberserie`, () => {
    //to avoid matches on Serial version UUID that happens to have a pnr-pattern inside it.
    expect(Pnr.regex.test(`1905197073153504885L`)).toBeFalsy();
    expect(Pnr.regex.test(`+460311234567`)).toBeFalsy();
  });
});
